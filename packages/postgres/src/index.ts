...
import type { AddEvidenceInput, AddMemoryInput, CreateDocumentChunkInput, CreateDocumentInput, CreateEdgeInput, CreateIngestionJobInput, CreateNodeInput, DataLayerProvider, DocumentChunk, DocumentChunkStore, DocumentId, DocumentStore, EdgeId, Evidence, EvidenceListInput, EvidenceStore, GraphEdge, GraphNode, GraphStore, GraphTraversal, IdentityBinding, IdentityStore, IngestionJob, IngestionJobStore, JsonObject, MemoryRecord, MemorySearchInput, MemoryStore, NeighborsInput, NodeId, NodeMergeRecord, PlanetDocument, PlanetIdentity, PlanetScope, SqlQueryInput, SqlQueryResult, SqlStore, SqlTransaction, TraverseInput, UpdateEdgeInput, UpdateIngestionJobInput, UpdateNodeInput, VectorRecord, VectorSearchInput, VectorSearchResult, VectorStore } from "@unknown-planet/core";
...
export class PgVectorStore implements VectorStore { constructor(private readonly db: PostgresDatabase, private readonly collections: Readonly<Record<string, VectorCollectionConfig>> = {}) {} private validate(namespace: string, embedding: number[], model?: string) { const collection = this.collections[namespace]; if (collection && embedding.length !== collection.dimensions) throw new Error(`Vector namespace '${namespace}' requires ${collection.dimensions} dimensions.`); if (collection?.model && model && collection.model !== model) throw new Error(`Vector namespace '${namespace}' requires model '${collection.model}'.`); } async upsert(input: VectorRecord): Promise<void> { this.validate(input.namespace, input.embedding, input.model); await this.db.query(`INSERT INTO vectors (scope_id,id,namespace,model,embedding,metadata) VALUES ($1,$2,$3,$4,$5::vector,$6::jsonb) ON CONFLICT (scope_id,namespace,id) DO UPDATE SET model=EXCLUDED.model,embedding=EXCLUDED.embedding,metadata=EXCLUDED.metadata,updated_at=now()`, [scopeId(input.scope), input.id, input.namespace, input.model ?? this.collections[input.namespace]?.model ?? null, vectorLiteral(input.embedding), JSON.stringify(input.metadata ?? {})]); } async search(input: VectorSearchInput): Promise<VectorSearchResult[]> { if (input.namespace) this.validate(input.namespace, input.embedding, input.model); const values: unknown[] = [scopeId(input.scope), vectorLiteral(input.embedding)]; const clauses = ["scope_id=$1"]; if (input.namespace) { values.push(input.namespace); clauses.push(`namespace=$${values.length}`); } if (input.model) { values.push(input.model); clauses.push(`model=$${values.length}`); } if (input.metadata) { values.push(JSON.stringify(input.metadata)); clauses.push(`metadata @> $${values.length}::jsonb`); } values.push(input.limit ?? 20); const result = await this.db.query<{ id: string; namespace: string; score: number; metadata: JsonObject }>(`SELECT id,namespace,1-(embedding <=> $2::vector) AS score,metadata FROM vectors WHERE ${clauses.join(" AND ")} ORDER BY embedding <=> $2::vector,id LIMIT $${values.length}`, values); return result.rows; } async delete(input: { id: string; namespace: string; scope?: PlanetScope }): Promise<void> { await this.db.query("DELETE FROM vectors WHERE scope_id=$1 AND namespace=$2 AND id=$3", [scopeId(input.scope), input.namespace, input.id]); } }
